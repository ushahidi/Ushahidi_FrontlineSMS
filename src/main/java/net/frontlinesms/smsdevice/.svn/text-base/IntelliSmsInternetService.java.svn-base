...
public class IntelliSmsInternetService extends AbstractSmsInternetService implements PopMessageProcessor {
		...
		try {
			...
			if (code == ResultCodes.OK) {
				...
				if (code == ResultCodes.InsufficientCredit) {
					statusString = FrontlineSMSConstants.Dependants.STATUS_CODE_MESSAGES[STATUS_LOW_CREDIT].replace(FrontlineSMSConstants.ARG_VALUE, String.valueOf(getRemainingCredit())) + ".";
					fireEvent();
				}
			...
			message.setStatus(Message.STATUS_FAILED);
			LOG.debug("Failed to send message [" + message + "]", e);
	...
	public void setUseForReceiving(boolean use) {
		this.settings.set(PROPERTY_USE_FOR_RECEIVING, new Boolean(use));
		persistedProperties.put(PROPERTY_USE_FOR_RECEIVING, String.valueOf(use));
	}
