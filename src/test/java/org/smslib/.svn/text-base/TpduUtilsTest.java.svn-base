...
public class TpduUtilsTest extends BaseTestCase {
	private static final Logger LOG = Logger.getLogger(TpduUtilsTest.class);
...
//> Message constants.  These are mostly for convenience.
				...
				for(short destinationPort : PORTS) {
					for(String recipient : MSISDNS) {
						for(int mpRefNo : MP_REF_NO) {
							...
							String[] pdus = TpduUtils.generatePdus_ucs2(messageText, "", recipient, mpRefNo, sourcePort, destinationPort, true, 0, 0, TpduUtils.getDcsByte(MessageEncoding.EncUcs2));
		...
		for (int j = 0; j < 320; j++) {
			...
			for(short sourcePort : PORTS) {
				for(short destinationPort : PORTS) {
					for(String recipient : MSISDNS) {
						for(int mpRefNo : MP_REF_NO) {
							String[] pdus = TpduUtils.generatePdus_8bit(bytes, "", recipient, mpRefNo, sourcePort, destinationPort, true, 0, 0, TpduUtils.getDcsByte(MessageEncoding.Enc8Bit));
							String rebuiltHex = "";
							for(String pdu : pdus) {
								CIncomingMessage message = new CIncomingMessage(convertOutgoingToIncoming(pdu), 0, "");
		...
		for(String messageText : TEXTS) {
			for(short sourcePort : PORTS) {
				for(short destinationPort : PORTS) {
					for(String recipient : MSISDNS) {
						for(int mpRefNo : MP_REF_NO) {
							String[] pdus = TpduUtils.generatePdus_gsm7bit(messageText, "", recipient, mpRefNo, sourcePort, destinationPort, true, -1, 0, TpduUtils.getDcsByte(MessageEncoding.Enc7Bit));
							String rebuiltText = "";
	...
	private String convertOutgoingToIncoming(String pdu) {
		...
		try {
			...
			byte[] userData;
			{
				ByteArrayInputStream in = new ByteArrayInputStream(HexUtils.decode(pdu));
	...
	private void testSemiOctetEncoding(String address, boolean isSmscNumber) throws IOException {
		...
		ByteArrayInputStream in = new ByteArrayInputStream(encodedMsisdn);
