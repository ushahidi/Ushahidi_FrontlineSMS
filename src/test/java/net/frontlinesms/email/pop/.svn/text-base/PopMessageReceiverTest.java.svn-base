 ...
 * @author Alex
 */
public class PopMessageReceiverTest extends BaseTestCase {
	/**
	 * Unit tests for {@link PopMessageReceiver#getMessageText(javax.mail.Multipart, String)}.
	...
	public void testGetMessageTextFromMultipart() throws MessagingException, IOException {
		...
		mult.addBodyPart(bp);
		String text = PopMessageReceiver.getMessageText(mult, desiredContentType);
		...
		mult.addBodyPart(bp);
		text = PopMessageReceiver.getMessageText(mult, desiredContentType);
	...
	/**
	 * Unit tests for {@link PopMessageReceiver#getSender(javax.mail.Message)}.
	 * @throws MessagingException 
	 */
	public void testGetSender() throws MessagingException {
		...
		String sender = PopMessageReceiver.getSender(message);
		...
		sender = PopMessageReceiver.getSender(message);
	...
	/**
	 * Unit tests for {@link PopMessageReceiver#getMessageText(javax.mail.Message)}.
	 * @throws MessagingException 
	 * @throws IOException 
	 */
	public void testGetMessageTextFromMessage() throws MessagingException, IOException {
		...
		String text = PopMessageReceiver.getMessageText(message);
		assertEquals("Checking getting a text from a non-multipart message, with string content.", content, text);
		
		// No content
		message = new MockMailMessage();
		text = PopMessageReceiver.getMessageText(message);
